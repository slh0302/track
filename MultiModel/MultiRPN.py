	...
	def define(self, immediateSize, weightDecay):
		with tf.name_scope('RPN'):
			with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(weightDecay), padding='SAME'):
				...
				self.boxes = BoxUtils.nnToImageBoxesMultiList(x_raw, y_raw, w_raw, h_raw, self.wA, self.hA, self.inputDownscale, self.offset, self.batch)
	...
	def loss(self, refBoxes):
		...
		def calcAllLosses(refAnchors, boxes, rawSizes, scores, boxSizes, refbx):
			with tf.name_scope('calcAllRPNLosses'):
				...
				p = tf.cond(tf.shape(positiveIndices)[0]>0, lambda:getPositiveLoss(boxes, rawSizes, boxSizes, positiveIndices, bestIou, classificationLoss, refbx), lambda: emptyPositiveLoss())
		...
		def calcLoss():
			with tf.name_scope('calcRPNLoss'):
				...
				loss = []
				for bx, sc, bs, rs, nm in zip(boxList, scoList, boxSizeList, rawList, range(self.batch)):
					...
					positiveLosses, negativeLosses = calcAllLosses(inAnchros, inBoxes, inRawSizes, inScores, inBoxSizes, bx)
				...
				return tf.reduce_mean(tf.stack(loss,axis=0))
	...
	def filterOutputBoxes(self, boxes, scores, others=[], preNmsCount=6000, maxOutSize=300, nmsThreshold=0.7): 
		with tf.name_scope("filter_output_boxes"):
			...
			for bx, sc in zip(boxList, scoreList):
				...
				sc, bx = tf.cond(tf.shape(sc)[0] > preNmsCount , lambda: tf.tuple(MultiGather.gatherTopK(sc, preNmsCount, [bx])), lambda: tf.tuple([sc, bx]))

				#NMS filter
				nmsIndices = tf.image.non_max_suppression(bx, sc, iou_threshold=nmsThreshold, max_output_size=maxOutSize)
