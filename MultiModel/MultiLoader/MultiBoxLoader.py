...
class MultiBoxLoader:
    ...
    def __init__(self, sources=[], initOnStart=True, batch=8):
        ...
        with tf.name_scope('dataset') as scope:
            ...
            self.enqueueOp = self.queue.enqueue([self.image, self.boxes, self.classes, self.number])
    ...
    def threadFn(self, tid, sess):
        ...
        while True:
            ...
            try:
                sess.run(self.enqueueOp,feed_dict={self.image:img, self.boxes:boxes, self.classes:classes, self.number:number})
    ...
    def get(self):
        images, boxes, classes, number = self.queue.dequeue()
        ...
        return images, boxes, classes, number
