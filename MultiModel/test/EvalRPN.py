...
categories = ['__background__','person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light', 'fire hydrant', 'stop sign', 'parking meter',
...
categorieOut = ["__background__", "car", "bus"]
...
with tf.Session(config=config) as sess:
    ...
    while begin < runTestNum:
        ...
        for i in range(batch):
            ...
            # img1 = np.copy(img2)
            # for bx in testSet[name]:
            #     cv2.rectangle(img1, tuple([bx[0], bx[1]]), tuple([bx[0] + bx[2], bx[1] + bx[3]]), (255,0,0), thickness=4)
            #     textpos = [bx[0], bx[1] - pad]
            #     cv2.putText(img1, bx[-1], tuple(textpos), font, fontSize, (255, 0, 0), thickness=fontThickness)
            # cv2.imwrite("./test/" + name + ".jpg", img1)
        ...
        for im, imName, rB, rC, rS, i, par, sList in zip(imList, resultList, rBoxes, rClasses, rScores, range(batch), param, sizeList):
            item = {}
            for bx, sc, cls in zip(rB, rS, rC):
                ...
                # output
                # """ out put """
                # cv2.rectangle(im, tuple(clipCoord(bx[0:2],im)), tuple(clipCoord(bx[2:4],im)), (255, 0, 0), thickness=4)
                # textpos = [int(bx[0]), int(bx[1] - pad)]
                # cv2.putText(im, categories[cls], tuple(textpos), font, fontSize, (255, 0, 0), thickness=fontThickness)
                # """   end   """
                boxCoord = originCoord(bx[0], bx[1], bx[2], bx[3], par[0], par[1], par[2], sList)
                if categories[cls] in categorieOut:
                    if imName in boxesRes[categories[cls]].keys():
                        boxesRes[categories[cls]][imName].append( boxCoord + [float(sc)])
                    else:
                        boxesRes[categories[cls]][imName] = [boxCoord + [float(sc)]]
                else:
                    print(categories[cls], bx, imName)
            # if imName in boxesRes[categories[cls]].keys():
            #     boxesRes[categories[cls]][imName].append(clipCoord(bx[0:2]) + clipCoord(bx[2:4]) + sc)
            # else:
            #     boxesRes[categories[cls]][imName] = [clipCoord(bx[0:2]) + clipCoord(bx[2:4]) + sc]

eva = eval(categorieOut, "/home/slh/tf-project/track/MultiModel/TFRecord", "test")
eva.evaluate_detections(boxesRes, '/home/slh/tf-project/track/MultiModel/test/output', ansname=TEST_SET)
