...
class RPN:
    def __init__(self, nClass, rpnLayer, rpnDownscale, rpnOffset, featureLayer=None, featureDownsample=None, featureOffset=None, weightDecay=1e-6, hardMining=True):
        ...
        if featureOffset is None:
            ...
            featureOffset = rpnOffset

        with tf.name_scope("RPN_module"):
            self.rpn = BaseRPN(rpnLayer, immediateSize=512, weightDecay=weightDecay,
                               inputDownscale=rpnDownscale, offset=rpnOffset)
            self.BoxRegNet = BoxRegNet(featureLayer[0], nClass, downsample=featureDownsample,
    ...
    def getLoss(self, refBoxes, refClasses, refDisp=None):
        return self.rpn.loss(refBoxes) + self.BoxRegNet.loss(self.proposals, refBoxes, refClasses)
