from model.BoxRegNet import BoxRegNet
...
class MultiRPN:
    ...
    def loss(self, refBoxes):
        ...
        def getPositiveBoxes(boxes):
            with tf.name_scope('getPositiveBoxes'):
                ...
                maxIouRes = []
                bestIouRes = []
                for refB in refBoxList:
                    iou = BoxUtils.iou(boxes, refB)

                    # iou: two dims tensor, axis=1 is ref box's iou
                    ...
                    boxRes.append(oneIfPositive)
                    maxIouRes.append([maxIou])
                    bestIouRes.append([bestIou])
                # change to tensor
                boxRes = tf.stack(boxRes, axis=0)
                maxIouRes = tf.stack(maxIouRes, axis=0)
                bestIouRes = tf.stack(bestIouRes, axis=0)
                return boxRes, maxIouRes, bestIouRes
        ...
        def getPositiveLoss(boxes, rawSizes, boxSizes, positiveIndices, bestIou, classificationLoss):
            with tf.name_scope('getPositiveLoss'):
                positiveBoxes, positiveRawSizes, positiveBoxSizes, positiveRefIndices, positiveClassificationLoss = \
                    MultiGather.gatherList([boxes, rawSizes, boxSizes, bestIou, classificationLoss], positiveIndices)
    ...
    def filterCrossBoundaryBoxes(self, boxes, others=[], boxInsideRate=1.0):
        with tf.name_scope('filterCrossBoundaryBoxes'):
            ...
            return MultiGather.gather([boxes], okIndices) + MultiGather.gatherList(others, okIndices)

    def clipBoxesToEdge(self, boxes):
        with tf.name_scope("clipBoxesToEdge"):
            x0, y0, x1, y1 = tf.unstack(boxes, axis=2)
    ...
    def filterOutputBoxes(self, boxes, scores, others=[], preNmsCount=6000, maxOutSize=300, nmsThreshold=0.7):
        ...
        with tf.name_scope("filter_output_boxes"):
            ...
            # Remove empty boxes
            boxes, scores = BoxUtils.filterSmallBoxesList(boxes, scores)

            tmpBox = []
            tmpScore = []
            for bx, sco in zip(boxes, scores):
                s, b = tf.cond(tf.shape(sco)[0] > preNmsCount,
                                        lambda: tf.tuple(MultiGather.gatherTopK(sco, preNmsCount, [bx])),
                                        lambda: tf.tuple([sco, bx]))

                # NMS filter
                nmsIndices = tf.image.non_max_suppression(b, s, iou_threshold=nmsThreshold,
                                                      max_output_size=maxOutSize)
                nmsIndices = tf.expand_dims(nmsIndices, axis=-1)

                b, s = MultiGather.gather([b, s], nmsIndices)

                tmpBox.append(b)

                tmpScore.append(s)

            return tf.stack(tmpBox, axis=0), tf.stack(tmpScore, axis=0)
